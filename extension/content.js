...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'gbrowse-status-pill';
  ...
  statusPill.innerHTML = `<span class="gbrowse-pill-dot"></span> gbrowse${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'gbrowse-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'gbrowse-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'gbrowse-ref-panel';

  const header = document.createElement('div');
  header.className = 'gbrowse-ref-panel-header';
  header.textContent = `gbrowse refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'gbrowse-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'gbrowse-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'gbrowse-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'gbrowse-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'gbrowse-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'gbrowse-ref-panel-more';
...
// Do NOT dispatch gbrowse-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('gbrowse-extension-ready'));
    return;
  }
