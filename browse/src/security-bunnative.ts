...
export function getCachedTokenizer(): TokenizerState {
  ...
  const gbrowseHome = process.env.GBROWSE_HOME ?? process.env.GSTACK_HOME ?? path.join(os.homedir(), '.gbrowse');
  const dir = path.join(gbrowseHome, 'models', 'testsavant-small');
...
export async function classify(text: string): Promise<ClassifyResult> {
  ...
  const _gbrowseHome = process.env.GBROWSE_HOME ?? process.env.GSTACK_HOME ?? path.join(os.homedir(), '.gbrowse');
  env.localModelPath = path.join(_gbrowseHome, 'models');
