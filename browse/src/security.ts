 ...
 *
 * Cross-process state lives at ~/.gbrowse/security/session-state.json
...
export type LayerName =
  | 'testsavant_content'
  | 'deberta_content'        // opt-in ensemble layer (GBROWSE_SECURITY_ENSEMBLE=deberta)
...
export interface AttemptRecord {
  ...
  layer: LayerName;
  verdict: Verdict;
  gbrowseVersion?: string;
}

const SECURITY_DIR = path.join(os.homedir(), '.gbrowse', 'security');
 ...
 * ~/.gbrowse/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * gbrowse uses local-only attack logging; no external telemetry binary.
 */
 ...
 * GBROWSE_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. GBROWSE_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.GBROWSE_BASH_BIN ?? env.GSTACK_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `gbrowse-telemetry-log` is a shell script
 ...
 * Bun.which + GBROWSE_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard gbrowse install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of gbrowse-telemetry-log with the
 ...
 * gbrowse-telemetry-log (which respects the user's telemetry tier setting).
...
// per-tab file under ~/.gbrowse/security/decisions/, sidebar-agent polls
