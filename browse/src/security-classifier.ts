 ...
 *
 * To upgrade: bump this string, run `GBROWSE_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.gbrowse/models/testsavant-small/ on first use.
 *
...
const MODELS_DIR = path.join(
  process.env.GBROWSE_HOME ?? process.env.GSTACK_HOME ?? os.homedir() + '/.gbrowse',
  'models',
);
...
// GBROWSE_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = ((process.env.GBROWSE_SECURITY_ENSEMBLE ?? process.env.GSTACK_SECURITY_ENSEMBLE) ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if ((process.env.GBROWSE_SECURITY_OFF ?? process.env.GSTACK_SECURITY_OFF) === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'GBROWSE_SECURITY_OFF=1 — ML classifier kill switch engaged';
...
export function loadDeberta(onProgress?: (msg: string) => void): Promise<void> {
  if ((process.env.GBROWSE_SECURITY_OFF ?? process.env.GSTACK_SECURITY_OFF) === '1') return Promise.resolve();
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like gbrowse itself),
    ...
    // Env var GBROWSE_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = (process.env.GBROWSE_HAIKU_TIMEOUT_MS ?? process.env.GSTACK_HAIKU_TIMEOUT_MS)
      ? Number((process.env.GBROWSE_HAIKU_TIMEOUT_MS ?? process.env.GSTACK_HAIKU_TIMEOUT_MS))
      : 45000;
    setTimeout(() => {
