 ...
 *
 * The session cookie (gbrowse_picker) is isolated from the scoped token system.
...
/** Extract session ID from the gbrowse_picker cookie. */
function getSessionFromCookie(req: Request): string | null {
  const cookie = req.headers.get('cookie');
  if (!cookie) return null;
  const match = cookie.match(/gbrowse_picker=([^;]+)/);
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        ...
        return new Response(null, {
          ...
          headers: {
            ...
            'Set-Cookie': `gbrowse_picker=${session}; HttpOnly; SameSite=Strict; Path=/cookie-picker; Max-Age=3600`,
      ...
      return new Response('Access denied. Open the cookie picker from gbrowse.', {
        status: 403,
        headers: { 'Content-Type': 'text/plain' },
      });
    }
