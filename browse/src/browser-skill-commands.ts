 ...
 * and passes it via GBROWSE_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    GBROWSE_PORT and GBROWSE_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get GBROWSE_PORT +
 * GBROWSE_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'GBROWSE_TOKEN' || k === 'GSTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip GBROWSE_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.GBROWSE_PORT = String(opts.port);
  out.GBROWSE_SKILL_TOKEN = opts.skillToken;
  // Legacy aliases so skills written against the old gstack env names keep working.
  out.GSTACK_PORT = out.GBROWSE_PORT;
