/**
 * gbrowse CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .gbrowse/browse.json for port + token
...
function chromiumProfileDir(): string {
  return path.join(process.env.GBROWSE_HOME || process.env.GSTACK_HOME || (process.env.HOME || '/tmp') + '/.gbrowse', 'chromium-profile');
}
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-gbrowse-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-gbrowse-browser' to restart.`);
    process.exit(1);
  }
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/gbrowse`;
      try {
