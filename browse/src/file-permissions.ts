/**
 * Cross-platform file permission restriction for sensitive gbrowse state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how gbrowse marks
 ...
 * install, sensitive gbrowse state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[gbrowse] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
