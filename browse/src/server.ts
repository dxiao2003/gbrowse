 ...
 *
 * State:
 *   State file: <project-root>/.gbrowse/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.gbrowse/browse-{console,network,dialog}.log
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const gbrowseHome = process.env.GBROWSE_HOME || process.env.GSTACK_HOME || path.join(home, '.gbrowse');
  const ngrokEnvPath = path.join(gbrowseHome, 'ngrok.env');
...
// so this branch is the normal path for /open-gbrowse-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-gbrowse-browser sessions.
...
export async function start() {
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.gbrowse/ngrok.env');
    } else {
