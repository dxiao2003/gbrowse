 ...
 *   2. git rev-parse --show-toplevel → projectDir/.gbrowse/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .gbrowse/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.gbrowse');
...
/**
 * Create the .gbrowse/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .gbrowse/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.(gstack|gbrowse)\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.gbrowse/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Resolve the gbrowse home directory.
 *
 * Resolution order (back-compat env vars honored):
 *   1. GBROWSE_HOME env
 *   2. GBROWSE_STATE_ROOT env
 *   3. GSTACK_HOME env (legacy back-compat)
 *   4. $HOME/.gbrowse (default)
 */
export function resolveGbrowseHome(): string {
  return (
    process.env.GBROWSE_HOME ??
    process.env.GBROWSE_STATE_ROOT ??
