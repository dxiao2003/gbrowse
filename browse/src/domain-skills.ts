 ...
 *
 * Scope:
 *   - per-project: ~/.gbrowse/projects/<slug>/learnings.jsonl
 *   - global:      ~/.gbrowse/global-domain-skills.jsonl
 *
...
function gbrowseHome(): string {
  return process.env.GBROWSE_HOME ?? process.env.GBROWSE_STATE_ROOT ?? process.env.GSTACK_HOME ?? path.join(os.homedir(), '.gbrowse');
}

function globalFile(): string {
  return path.join(gbrowseHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(gbrowseHome(), 'projects', slug, 'learnings.jsonl');
}
