...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .gbrowse/browse-*.log');
  process.exit(1);
}
...
export class BrowserManager {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const userDataDir = path.join(process.env.GBROWSE_HOME || process.env.GSTACK_HOME || (process.env.HOME || '/tmp') + '/.gbrowse', 'chromium-profile');
      fs.mkdirSync(userDataDir, { recursive: true });
