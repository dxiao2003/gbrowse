 ...
 *
 * /skillify stages a candidate skill into ~/.gbrowse/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.gbrowse/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(process.env.GBROWSE_HOME ?? process.env.GSTACK_HOME ?? path.join(os.homedir(), '.gbrowse'), '.tmp');
