...
const TMP_HOME = path.join(os.tmpdir(), `gbrowse-telemetry-test-${process.pid}-${Date.now()}`);
const TELEMETRY_FILE = path.join(TMP_HOME, 'analytics', 'browse-telemetry.jsonl');

// Use GBROWSE_HOME env to redirect telemetry writes (read each call,
// not cached at module-load).
process.env.GBROWSE_HOME = TMP_HOME;
process.env.GBROWSE_TELEMETRY_OFF = '0';
...
describe('telemetry: signals fire to ~/.gbrowse/analytics/browse-telemetry.jsonl', () => {
  ...
  it('GBROWSE_TELEMETRY_OFF=1 silences all events', async () => {
    process.env.GBROWSE_TELEMETRY_OFF = '1';
    const { logTelemetry, _resetTelemetryCache } = await import('../src/telemetry');
    _resetTelemetryCache();
