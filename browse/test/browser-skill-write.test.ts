 ...
 * never touch the user's real ~/.gbrowse/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.gbrowse/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.gbrowse/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.gbrowse', '.tmp');
});
